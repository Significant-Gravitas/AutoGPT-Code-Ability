import logging
from .model import ApplicationRequirements, APIRouteRequirement, RequestModel, ResponseModel, Parameter
...
def hardcoded_requirements(task: str) -> ApplicationRequirements:
    ...
    if task == 'Availability Checker':
        ...
        check_availability_request = RequestModel(
            ...
            params=[
                Parameter(name="current_time", param_type="datetime", description="The timestamp at which the availability status is being requested."),
                Parameter(name="schedule_data", param_type="List[Tuple[datetime, datetime]]", description="A list of tuples representing the schedule of the professional, where each tuple contains start and end times of appointments or busy periods.")
            ]
        )
        ...
        availability_status_response = ResponseModel(
            ...
            params=[
                Parameter(name="availability_status", param_type="str", description="The current availability status of the professional. Possible values are 'Available' and 'Busy'.")
            ]
        )
