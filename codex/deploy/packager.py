...
def generate_actions_workflow(application: Application) -> str:
    ...
    Args:
        application (Application): The application to be used to generate the workflow.yml file

    Returns:
        str: The workflow.yml file
    """
    logger.info("Generating workflow.yml file")
...
async def create_zip_file(application: Application) -> bytes:
    ...
    try:
        with tempfile.TemporaryDirectory() as package_dir:
            ...
            github_actions_directory = os.path.join(package_dir, ".github", "workflows")  # Define.github/workflows directory path
            os.makedirs(github_actions_directory, exist_ok=True)  # Create .github/workflows directory if it doesn't exist

            github_actions_deploy_file_path = os.path.join(github_actions_directory,"deploy.yml")
