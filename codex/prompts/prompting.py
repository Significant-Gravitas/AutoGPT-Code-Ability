...
class PromptEngine:
    ...
    def __init__(self, model: str, debug_enabled: bool = False):
        ...
        Args:
            ...
            debug_enabled (bool): Enable or disable debug loggerging.
        """
        self.model = model
        self.debug_enabled = debug_enabled
        if self.debug_enabled:
            logger.debug(f"Initializing PromptEngine for model: {model}")
        ...
        try:
            ...
            if self.debug_enabled:
                logger.debug(f"Using the closest match model for prompts: {self.model}")
    ...
    def load_prompt(self, template: str, **kwargs) -> str:
        ...
        try:
            ...
            if self.debug_enabled:
                logger.debug(f"Loading template: {template}")
            template = self.env.get_template(f"{template}.j2")
            if self.debug_enabled:
                logger.debug(f"Rendering template: {template} with args: {kwargs}")
