...
async def create_specification(
    ...
    create_db = (
        prisma.types.DatabaseSchemaCreateInput(
        ...
        if spec_holder.db_response
        else None
    )

    def get_api_route_spec(m: codex.requirements.agent.Module):
        route_specs = []
        for r in m.api_routes:
            spec = {
                "method": r.http_verb,
    ...
    if spec_holder.features:
        create_spec["Features"] = {
            "connect": [{"id": f.id} for f in spec_holder.features]
        }
    if create_modules:
        create_spec["Modules"] = {"create": create_modules}
    if create_db:
        create_spec["DatabaseSchema"] = {"create": create_db}
    if spec_holder.ids.user_id:
        create_spec["User"] = {"connect": {"id": spec_holder.ids.user_id}}
    if spec_holder.ids.app_id:
        create_spec["Application"] = {"connect": {"id": spec_holder.ids.app_id}}
    if spec_holder.ids.interview_id:
        create_spec["Interview"] = {"connect": {"id": spec_holder.ids.interview_id}}
